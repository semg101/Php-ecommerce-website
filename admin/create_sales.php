...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['sale_price'], $_POST['start_date'], $_POST['end_date'])) {
        ...
        foreach ($_POST['sale_price'] as $sku => $price) {
        	if (filter_var($price, FILTER_VALIDATE_FLOAT) && ($price > 0) && (!empty($_POST['start_date'][$sku])) && (preg_match('/^(201)[3-9]\-[0-1]\d\-[0-3]\d$/', $_POST['start_date'][$sku]))){
...
<h3>Create Sales</h3>
<p>To mark an item as being on sale, indicate the sale price, the date the sale
starts, and the date the sale ends. <strong>All dates must be in the format YYYY-MMDD.</strong> You may leave the end date blank, thereby creating an open-ended sale.
                ...
                <?php
                    ...
                    while ($row = mysqli_fetch_array ($r, MYSQLI_ASSOC)) {
                        echo '<tr>
                            ...
                            <td align="center"><input type="text" name="start_date[' . $row['sku'] . ']"/></td>
                            <td align="center"><input type="text" name="end_date[' . $row['sku'] . ']"/></td>
                            </tr>';
                    }
                ?>
            </tbody>
